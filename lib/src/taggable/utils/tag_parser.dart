...
TextSpan getTaggableSpan<T>({
  ...
  for (final match in getTagMatches(content, tagStyles)) {
    ...
    final textStyle = textStyleBuilder.call(context, tag.style.prefix) ?? style;
    ...
    // If the tag text contains a space marker, apply zero letter spacing
    final hasSpaceMarker = tagText.contains(spaceMarker);
    if (hasSpaceMarker) {
      spans.add(
        TextSpan(
          text: tagText,
